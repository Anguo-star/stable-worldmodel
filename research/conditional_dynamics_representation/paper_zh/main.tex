...
\paragraph{D2-v1 未进入行为实验。}
在独立的 Motion Damping--LeWM 预注册单元中，P1a 仅在 base forward/reverse 上确认接触介导的动作杠杆路由存在；P1b 的 $192$ 个冻结覆盖--动作槽位只填满 $128$ 个，\texttt{low\_approach} 为 $0/64$，故 D2-v1 在训练前关闭。该结果只否定此动作/几何/覆盖配方，不证明或证伪原生数据路线，也不改变 \benchmark 与 \method 的结论。
