# ContextWorld / COJA 实验档案

> **受众与用途：内部实验记录。** 本文件无损保留原 README 的完整历史内容、运行身份与证据索引；
> 它不是论文正文，也不是新读者入口。项目概览见 [README.md](README.md)，专家证据链见
> [TECHNICAL_REPORT.md](TECHNICAL_REPORT.md)，论文源文件见 [paper_zh/main.tex](paper_zh/main.tex)。

# 条件动力学 ICL：从边缘非坍缩到条件联合响应

> 面向外部读者的论文式技术报告见
> [TECHNICAL_REPORT.md](TECHNICAL_REPORT.md)。它按“问题定义—理论分析—根因拆分—方法—
> 跨任务验证—局限”组织，并纳入 2026-08-30 ActionDelay 完整终点与 Motion native-rollout
> 对照、2026-08-29 Contact Friction 与 Motion Damping 的完整 10-epoch 终点、2026-08-28 的统一完整训练对照以及 Motion/Contact hidden planning 与
> rollout-consistency 结果。本文件保留扩展实验、实现边界和历史证据，作为可复核的补充材料。
> 2026-08-31 启动的根因候选指标、首轮 Motion 实测、数据构造决策树与固定实验矩阵见
> [ROOT_CAUSE_DATA_STRATEGY_ZH.md](ROOT_CAUSE_DATA_STRATEGY_ZH.md)。承重 claim 的 scope、证据、
> 复现层级与降级条件见 [CLAIMS_LEDGER_ZH.md](CLAIMS_LEDGER_ZH.md)；当前总体状态为**第一轮机制
> 边界界定完成、统一低-`rho` 充分解释已证伪、通用根因未闭合**。

> **2026-08-30 ActionDelay 完整终点。** LeWM+COJA、training seed `3072` 已完成 `10`
> epoch，并在同一最终 checkpoint 上完成 Development ICL 与原始 TwoRoom CEM。Development
> 的 future/history/switch/worst=`0.882/0.918/1.000/0.763`，gain/alignment/NRE=
> `6.877/0.752/70.995`，joint-pair success=`0`。这组结果不能概括成“COJA 失败”：相对接近
> 随机且几乎不使用历史的原生 LeWM，模型已经明显学会根据历史分配正确 future；但预测响应的
> 幅值在长训练后严重过放大，方向大体正确而尺度校准失稳。准确表述是
> **assignment 已学会，response calibration 未闭合**。
>
> 同 checkpoint 的原始 TwoRoom CEM 在评测 seed `42--47` 上分别为
> `37/37/42/41/40/41` 个成功，每个 seed 50 条，合计 `238/300=79.3%`。这是标准环境的绝对
> 保持量，不是 ICL 指标，也不是缺少同配方 native 对照时的方法增量。此次只运行 Development，
> 未访问公开 Test。下一步先回放短预算到 epoch-10 的保存 checkpoint，定位响应开始过放大的
> 时间点，再判断需要响应归一化或训练期调度；不能从单一终点直接否定 COJA。
>
> **2026-08-30 Motion native-rollout 对照。** 在同一 training seed、完整 `10` epoch 的
> Development 机制实验中，仅使用原生 loss 的 `K=3` sliding/expanding rollout 都没有形成
> 可用的历史条件响应；单步 COJA 虽未达到理想校准水平，但方向明确优于两者：
>
> | method | future | history | gain ↑ | NRE ↓ | alignment ↑ | joint |
> |---|---:|---:|---:|---:|---:|---:|
...
>
> 六个标准 PushT 评测 seed、每个 50 条的 CEM 成功率为 expanding/sliding/COJA=
> `38.3%/42.7%/78.7%`。全部评测 target separation 已通过，因此已有证据同时否定两个当前
> 假设：主要问题不是长度为 3 的滑动窗口丢掉更早历史，扩大为动态历史也没有救回 ICL；单纯
> 增加原生多步预测误差也没有产生足够的条件辨识压力，不能替代 COJA。这不与 RC-COJA 结果
> 冲突：前者问“native rollout 能否从零建立条件对应”，后者问“已由 COJA 建立的对应能否传到
> self-rollout”。本组是单训练 seed、Development-only 的机制证据，不是正式发布分数。

> **2026-08-29 Motion Damping 完整终点。** LeWM+COJA seed `3072` 已从头完成统一
> `10`-epoch、8 卡 BF16、原始/ContextWorld `50/50` 训练，并在同一
> `weights_epoch_10.pt` 上完成 Development ICL 与标准 PushT CEM `6×50`。Development 为
> future/history/switch/worst=`0.510/0.568/0.754/0.277`，gain/alignment/NRE=
> `0.044/0.205/0.958`，joint-pair success=`0.016`；全部 target pair 非重合。标准 PushT
> CEM 六个评测 seed 的成功数为 `42/40/35/40/38/41`，合计 `236/300`。这是绝对保持结果，
> 不是方法增量，因为本地尚无同初始化、同数据、同完整预算的 native full-10-epoch 对照。
>
> 保存的 epoch `1/2/4/6/8/10` Development 轨迹显示 gain 从约 `0` 单调增至 `0.044`，NRE
> 从 `1.001` 降至 `0.958`，switch 从 `0.531` 增至 `0.754`。因此该运行不是早期学会后被完整
> 训练冲掉，而是统一从头训练配方只形成了温和的条件响应 onset。它弱于另一项严格 matched
> 8,192-step 证据，但不推翻后者：在那项实验中，同数据 native→COJA 的 gain 为
> `0.0065→0.370`、NRE 为 `1.130→0.767`，已经识别出明确方法提升。准确结论是“COJA 在
> Motion 上有严格 matched 正效应，但这一正效应尚未被当前 joint-scratch full-10 配方复现到
> 相同幅度”，而不是“COJA 没有提升”。当前 ContextWorld-v1 bundle 没有发布 Motion offline
> Test split，CLI 会明确拒绝 `--evaluation-split test`，所以这里不能伪造 Public Test 结果。
> 身份、训练曲线与 CEM 汇总见
> [机器可读摘要](artifacts/motion_damping_lewm_coja_seed3072_full10_summary.json)。
...
> RC 的增量 retention 代价。另一个同数据、同预算 matched native 为 `237/300`，不是公开原始
> checkpoint；一步 COJA 相对它的 `-10.33pp` 是需要明确报告的保持性代价，并与 RC 方法增量分开。
...
标准 PushT 的相同 300 queries 为 matched native/一步 COJA/RC=`237/206/207`。RC−一步 COJA 为
`+0.33 [-4.33,+5.00]pp`，discordant wins/losses=`25/24`，没有检测到 RC 方法代价；
一步 COJA−matched native 已为 `-10.33 [-15.33,-5.67]pp`，RC−matched native 为
`-10.00 [-15.00,-5.00]pp`。因此约 `10pp` 是一步 COJA 已经引入的保持性代价，不能再次
误归因给 rollout 方法，也不能归为公开 checkpoint 或共享数据路径差异。Motion action-support
复核与完整单阶段训练已在 §5.43 完成；发现期
...
### 5.45 ActionDelay：完整训练保留分配能力，但响应尺度漂移

ActionDelay 的 LeWM+COJA seed `3072` 已完成 `10` epoch。最终 Development 的
future/history/switch/worst 为 `0.882/0.918/1.000/0.763`，说明模型已能用历史选择对应的
future；alignment=`0.752` 也表明总体方向不是随机的。与此同时，gain=`6.877`、NRE=`70.995`
且 joint-pair success=`0`，说明响应幅值相对真实目标严重放大。这个终点暴露的是
**assignment--calibration gap**，不是“模型没有学到 ICL”，更不能覆盖此前短预算 COJA 相对
原生 LeWM 的明显提升。

同 checkpoint 的原始 TwoRoom CEM 在六个评测 seed 上分别成功
`37/37/42/41/40/41` 次，合计 `238/300=79.3%`。该结果只度量标准环境绝对保持；在缺少同配方
native 完整终点时不能承担方法归因。本轮只使用 Development，公开 Test 保持未访问。

十个保存 checkpoint 已在同一冻结 Development 上用 CPU 完成零训练步重评分。关键断点如下；
表中 epoch 10 是同设备轨迹值，原始 GPU 终点 `gain/NRE=6.877/70.995` 仍是终点来源记录。
...
因此 epoch 2 已形成强 assignment，epoch 3 的保存点校准最好，epoch 4 首次明确退化且主要由
正交残差而非纯尺度误差驱动，epoch 5 起在 assignment 仍强时进入严重过响应。这直接支持
“先学会 ICL 分配，长训练后校准漂移”，而不是 COJA 没学会 ICL。该轨迹是回顾性诊断，不能把
epoch 3 事后升格为正式发布分数；任何提前停止、归一化或权重调度都必须另行预注册验证。

### 5.46 动态历史窗口与原生三步 rollout 均不能替代 COJA

Motion Damping 的完整 `10`-epoch 单 seed Development 对照固定训练终点，只改变历史组织与
训练目标：`sliding` 始终保留最近 3 个 token，`expanding` 动态保留更早历史，两者都使用
`K=3` 原生 rollout；COJA 分支使用单步条件目标。

| method | future | history | gain ↑ | NRE ↓ | alignment ↑ | joint-pair |
|---|---:|---:|---:|---:|---:|---:|
| native `K=3` sliding | `0.504` | `0.504` | `0.0014` | `1.0018` | `0.0205` | `0` |
| native `K=3` expanding | `0.500` | `0.492` | `-0.0009` | `1.0089` | `-0.0112` | `0` |
| 单步 COJA | `0.510` | `0.568` | `0.0437` | `0.9579` | `0.2054` | `0.0156` |

两种 native rollout 都接近“输出不随历史变化”的 `NRE=1` 参照点，expanding 甚至没有方向性
收益。由于 evaluation target separation 已通过，不能把结果解释为不同历史对应的真实 future
不可区分。标准 PushT CEM 的六评测 seed 汇总为 expanding=`38.3%`、sliding=`42.7%`、
COJA=`78.7%`；它说明当前 COJA checkpoint 的原任务绝对保持也明显更好，但这里没有训练 seed
重复，不能把评测 episode 当成方法重复。

因此本实验支持三个范围严格的结论：历史窗口截断不是主要根因；误差积累本身没有把隐藏动力学
差异变成足够强的优化压力；多步 native rollout 不能替代逐条件对应信号。当前不追加同一
`K=3` 配方的 `COJA+rollout` 组合臂，先量化 native loss 的条件压力。已有 RC-COJA 仍是另一项
有效结论：在条件对应已经建立后，support-matched 自回归监督可以把它传向更长规划视界。

§5.45 full-10 ActionDelay 的训练身份、最终 Development、六个 CEM cell 以及 epoch 1--10
checkpoint 哈希已归档为
[终点最小机器回执](artifacts/action_delay_lewm_coja_seed3072_full10_summary.json)，十个 epoch 的
Development 重评分与分解见[轨迹回执](artifacts/action_delay_lewm_coja_seed3072_full10_trajectory/summary.json)。本节
sliding/expanding 两臂的训练身份、20 个 checkpoint 哈希、epoch 1/2/3/10 Development 结果及
12 个 CEM cell 也已归档为
[最小机器回执](artifacts/motion_damping_native_rollout_full10_summary.json)；生成回执未增加训练或
评测计算。

### 5.47 根因工作假设：native loss 含有响应项，但可能被中心误差与采样噪声淹没

对共享同一 `(Q,A)` 的二元组，令预测和目标分别为 `p_0,p_1` 与 `t_0,t_1`，组中心为
`bar p,bar t`，响应为 `Delta p=p_1-p_0`、`Delta t=t_1-t_0`。逐样本 native MSE 可精确写成
...
所以“native loss 完全没有历史信号”并不准确；它在精确重叠组上隐式包含条件响应项。当前最强、
但尚待验证的解释是：该项没有按 target-response energy 归一化，又只占总体样本与坐标误差的一小
部分；优化器更容易先降低共享中心、静态背景和一般动力学误差，有限批次中成对响应梯度还会被
非条件噪声抵消。扩大历史窗口只增加可用信息，增加 rollout 只放大总预测误差，两者都不改变这项
相对权重，因此现有负对照与该解释一致，但尚未单独证明它。

下一阶段冻结以下诊断，不先开发新 loss：

1. **正确--交换历史 loss gap**：逐 query 报告
   `G_swap=L_native(H_swap,Y)-L_native(H_correct,Y)` 的均值、中位数、分位数与正号比例。二元
   squared error 下，`G_swap=<Delta p,Delta t>`；接近零可区分为 `Delta p` 近零、与
   `Delta t` 近乎正交，或正负 query 抵消，不能只看总体均值。
2. **条件监督质量**：统计精确/近邻 `(Q,A)` overlap 覆盖率、每组历史平衡、history probe、
   target separation，并定义条件能量占比
   `rho_cond=E||Delta t||^2/(4*E||t-E[t]||^2)`。按任务、视界、动作 leverage、响应幅值与 query
   密度分层，先检验 `rho_cond` 的跨任务排序是否与 native 可学性一致；若排序不一致，“条件项能量
   太小”就不能作为主根因。
3. **优化可见性**：利用上式分别反传 center 与 response 两项，按 Predictor block 记录梯度范数
   比、余弦与跨 batch 信噪比；prediction/output space 与 parameter space 分开报告，因为前者
   的有利方向经过共享 Predictor Jacobian 后可能变成参数冲突。在保存 checkpoint 上追踪这些量，
   定位“没有压力”“梯度冲突”与“后期尺度漂移”。
4. **数据优先的 native-only 矩阵**：先完成上述零训练统计，再固定初始化、transition 数、优化步
   与自然分布验证集，比较当前 paired 数据、按 response/action leverage 分层重采样、扩大高
   `rho_cond` 的 `(Q,A)` 采集，以及“高辨识样本启动 + 宽 query/action 自然幅值混合校准”。既有
   twin-sampler 已表明只提高 pair co-batching 密度不能救回 native ICL，因此不重复训练该臂；
   COJA 只作正对照，不参与数据候选选择。

这组实验将把 COJA 的针对性目标还原成更一般的数据原则：**条件重叠、组内动力学平衡、具有足够
作用量的查询动作、跨部署支撑的 query 覆盖，以及高辨识样本与自然幅值样本的校准混合**。若只改
数据就能稳定扩大 native `G_swap`、使 Development gain/alignment 为正且 NRE 降到 1 以下，
后续主线应优先转向数据构造；若 target separation 与 overlap 充分而 response-gradient ratio
仍接近零，才说明需要显式条件加权或结构化目标。ActionDelay 若先学对分配、随后 response norm
持续发散，则单独归入尺度调度问题，不能与“完全学不到历史”混为一个根因。

### 5.48 首轮原生条件压力实测：弱能量、参数衰减与覆盖差距同时存在

2026-08-31 已把 §5.47 的前三项诊断实现为 task-neutral paired 指标和 Motion 的两个只读
runner。所有新运行均为零 optimizer step；训练梯度 runner 的 fail-closed guard 记录
optimizer constructor/step、Development/Public scorer 和非训练 benchmark read 全为 `0`。
公开 Test 未访问。

同一冻结训练 batch 含 `32` 个完整 Motion condition pairs。latent `rho_cond` 在共同初始化和
native step-8192 均为 `0.291%`。初始化时 response/nonconditional 参数梯度范数比为 `3.92%`，
两项平方能量占比 `0.153%`，response `Bcrit=29.59`、`SNR(32)=1.04`；native 终点相应为
`5.57%/0.309%/26.56/1.10`。训练期间 hidden center MSE 从 `0.04532` 降至 `0.02033`，但
response MSE 从 `0.003083` 微升至 `0.003207`。因此总 native loss 的改善主要来自中心预测，
不是因为 response 项完全不存在，也不是长训练后才突然丢失条件梯度。

在独立 Motion Development pairs 上，native/COJA 使用相同 target encoder，latent
`rho_cond` 同为 `0.228%`。native 的 `G_swap` 均值/中位数为
`4.67e-5/-1.09e-4`，正号比例 `44.1%`，gain/NRE=`0.0065/1.1298`；sign-flip 与
cross-query null 均不显著。COJA 的对应值为 `2.666e-3/2.225e-3`、`96.9%` 和
`0.3696/0.7665`，两项随机化检验均达到 Monte Carlo 下限 `p=0.000488`。这不是 COJA
失败，而是它在同一弱条件能量上形成了 native 没有形成的一致 query-specific response。

native 在冻结训练 batch 上已把平均 `G_swap` 从初始化的 `-3.59e-4` 推到 `+7.19e-4`，但
Development 上又接近零，提示后续不能只重复训练模板。Development 按 latent target-response
energy 分层时，最低到最高四分位的 `rho_cond` 从 `0.087%` 升至 `0.473%`，native
`G_swap` 均值从 `-2.56e-4` 变为 `+4.24e-4`，支持优先测试高辨识度数据；但 query action
norm 在该任务全部为零，动作 leverage 必须定义为状态依赖的 counterfactual effect，不能直接用
动作幅值。

这仍是一个任务、一个冻结训练 batch 的机制证据，不能据此把“低 rho”写成通用主因。但继续扩展
零训练指标不作为数据实验的前置门：先用最小 `D1+native` 干预检验因果方向，再决定是否值得在
native 正例、native 负例和尺度漂移例上扩展多 batch 统计。完整决策路由、数据矩阵与资产链接见
[ROOT_CAUSE_DATA_STRATEGY_ZH.md](ROOT_CAUSE_DATA_STRATEGY_ZH.md)。

### 5.49 双路线决策：先冻结 D0，再补 D1+native

当前不先调整 COJA 的训练数据。这样做会把 objective-side 的 COJA 效应与 data-side 的分布效应
混在一起，无法回答未来主模型是否可以只靠预训练数据获得历史使用能力。执行顺序固定为：

1. 冻结 benchmark v1 的自然评测协议与当前自然训练分布 `D0`，归档已有
   `D0+native/D0+COJA`；
2. COJA 的 matched control、机器身份和 ActionDelay 短预算到 full-10 校准轨迹已经补齐；暂停新的
   loss、COJA+rollout、多 seed 和跨模型族扩展；
3. 从相同原始候选池构造等样本、等步数的高辨识 `D1`，首先只跑 `D1+native`；
4. 在冻结自然 Development 上检查 history deletion/swap drop、gain/NRE 与原环境保持；
5. 只有 `D1+native` 在 held-out query 上有效，才补 `D1+COJA` 并据交互结果决定论文主次。

`D1+native` 若基本替代 COJA，则数据原则升为主贡献而 COJA 保留为可诱导性证明；若两者叠加，
同篇写成互补机制；若数据效果局部或不稳定，当前投稿仍以 benchmark+COJA 为主，数据路线独立进入
后续主模型预训练。只改变训练分布时维持 benchmark v1，新增 training-distribution track；新增
评测 split、任务或指标时才称 v2。完整分流表见
[ROOT_CAUSE_DATA_STRATEGY_ZH.md](ROOT_CAUSE_DATA_STRATEGY_ZH.md#8-两条路线与执行优先级)。

截至 2026-08-31，四个 LeWM 原生困难任务的 `D0+COJA` 完整训练均已结束，后续不再补 COJA
训练；ActionDelay full-10 的终点与逐 epoch Development 轨迹也均已归档。DINO-WM、LeWM、PLDM
的 `D0+native` 云侧队列与 D1 数据构造并行推进；`D1+native` 是下一项新增训练，额外 COJA
seeds、`D1+COJA` 和 COJA+rollout 均等待该结果再决定。

D1 的首个可执行配方已冻结为 Motion `D1-E50 + native`：按完整 forward/reverse×mode twin
group 重排 Training 曝光，保留 50% natural anchor，并只用 Training manifest 的物理 future
响应选择高辨识池。现有 synthetic query action 全为零，因此独立 action-leverage 构造明确留到
...
### 5.58 机制边界反转矩阵与 D2 前最小 comparator
...
| ActionDelay | `5120` | `35.72917` | `1.00000`（退化） | `5.16734e-4` | `0.35956 [0.35945,0.35969]` | observed 11-condition groups |
| Speed | `4096` | `23.15537` | `0.16933` | `3.28594e-4` | `0.15381 [0.14272,0.16441]` | simulator counterfactual on Training queries |

ActionDelay 的 `rho_phys,64=1.00000` 必须标为**退化分母**：该任务的 `B_32/B_64/B_128` 在全部
`5,120` 个 query 上 mean/std/全部分位数均为 `0`，`C_phys` 亦为常数 `35.72917`（std `7.11e-15`），
因此 `rho=C/(C+0)=1` 是构造结果而非高条件份额。该数值自本条起**退出跨任务强弱比较**；ActionDelay
可用的只有 `C_phys` 的描述性数值与 raw-pixel `rho_pixel,64=0.35956`，后者亦不设跨任务阈值。
...
上游因素。但不允许写成充分因果：Speed 的证据不是 observed twins；ActionDelay 的 raw-pixel 份额
明显不低仍为 LeWM 负/PLDM 正（其 physical `rho` 因退化分母不可用）；Action Strength 上游最强之一
仍为 LeWM 正/PLDM 负。Motion 的 history `H_pixel` 还高于 Action Strength，说明“历史 clue 看不见”
与“历史对 future loss 的结果差异太弱”不能混为一谈。

因此本条冻结的是**第一轮机制边界**，不是根因收口：**统一低-`rho` 充分解释已被两组同数据跨模型
反转证伪；通用根因未闭合。** 当前允许的最强通用表述只是候选机制——**有效条件可见性/可迁移性
不足**：条件信号可能在 raw target、训练 latent、loss/Jacobian 或跨 query 梯度聚合任一层衰减，
binding cause 依赖具体 task/model/init。数据分布是可优先操作的上游层，但 universal data-only
claim 已被否定。
...
在同一 LeWM 实现路径上，A3（PLDM-active objective）相对 native A0 的 step-0 沿 response-residual
有利方向的一阶变化**绝对值**从 `-135.214` 变为 `-6569.057`（倍数 `48.5828`），全参数 total-gradient
范数比为 `2.8006`，因此**按总梯度范数归一化后的方向效率约 `17.35x`**；A3 与 native PLDM A4 在该
一阶量上的相对差为 `7.47e-6`。到 step 256，A3/A0 的 target-latent matched-to-unrelated ratio 为
`1.7019x`。三 seed 均值如下：
...
因此 ActionDelay 的首个局部分离可定位到 **objective/regularizer-induced gradient routing**。三条
边界同时记录：一阶量是局部线性化，其绝对尺度依赖各 objective 的整体 loss scale，`48.5828x` 不能读
成“48 倍学习速度”，归一化后的 `17.35x` 才是方向质量口径；所有 arm 的 signed gain 仍为负，故这是
bounded mechanism gate，既不是 256-step history-use 正例，也**不能称目标路由是唯一瓶颈或已观察到
行为反转**；raw data 与 step-0 probe latent 在本门中是逐元素固定的受控量，因此不可能是本门内的分离
来源，这**不**等于数据因素在 ActionDelay 或其他任务中被普遍否定。共同初始化只指 shared core，不
声称完整 A0/A4 参数集合同构。

Action Strength exact-batch 反向确认未成立，该单元记为 **unlocalized downstream bottleneck**。
LeWM/PLDM 使用不同预训练初始化，condition-pair prediction coherence 为 `0.2365/0.2420`，两者
native total gradient 均局部提高 signed gain；PLDM 终点在该 Training batch 上 gain/NRE=
`0.7337/0.2203`，仍对应冻结 held-out 负标签。因此不能事后选择跨模型零步阈值；未解释部分位于该
Training batch 下游，`coverage / 迁移 / 校准` 只是候选集合，没有证据指向其中任何一项。

本条的复现层级是 **`L1` 聚合可重复**：在 `30` 个冻结输入（另加 config 与汇总器源码 SHA）上重跑
汇总器得到字节一致的 `summary.json` 与 `per_cell.jsonl`，回执记录 `model_loaded=false`、
`optimizer_steps=0`。它**没有重跑任何上游训练**，因此不是训练复现、统计复现或因果复现；配套单元
测试属于软件合同证据，只说明汇总器行为符合契约，不构成科学结论证据。
...
结论冻结为：低像素/数据份额是 Motion 的可信上游因素，但不是统一充分根因；第一轮完成的是机制边界
界定，通用根因未闭合，一般机制只写成候选——条件信号可能在数据、表示、objective/Jacobian、梯度
聚合、coverage 或 calibration 任一层失去有效可见性/可迁移性。反转矩阵本身是 factorial/triangulation
设计而非独立复制：正式 inventory 的 8 个 cell 中只有 `1` 个 four-layer-complete，
`all_cells_four_layer_complete=false`，不存在“3/4 单元一致即可确认”的统计规则。
下一步只做 Motion D2-0 新轨迹构造门（已获授权，未预设成功），不继续调整 D1 重复率。
...
### 5.64 claim 收紧与承重台账建立

2026-09-02 建立 [`CLAIMS_LEDGER_ZH.md`](CLAIMS_LEDGER_ZH.md)，把 12 条承重 claim 的 statement、
scope、证据类型/artifact、seed/评测单元、不确定性、门状态、复现层级、未排除的替代解释、降级触发
条件与出现位置固定为可维护台账，并同步收紧 README、`ROOT_CAUSE_DATA_STRATEGY_ZH.md`、
`ROOT_CAUSE_REVERSAL_MATRIX_PROTOCOL_ZH.md` 与 `D1_CONSTRUCTION_PLAN_ZH.md` 的措辞。本次只改文档，
不新增实验、不修改任何 artifact、config、script 或 test。

四处主要收紧：

1. “通用/统一根因已冻结、已收口”改为**第一轮机制边界界定完成；统一低-`rho` 充分解释已证伪；
   通用根因未闭合**；
2. D1 结论统一命名为**方向性数据干预证据 / upstream-to-gradient directional transmission**，
   不称 native behavior reversal；D2 记为已获授权但未预设成功；
3. ActionDelay 的 physical `rho=1` 标为退化分母并退出跨任务比较；模型侧结论改写为
   objective/regularizer-induced gradient routing 的强局部支持，同时记录绝对值、倍数与 scale
   caveat；Action Strength 记为 unlocalized downstream bottleneck；
4. 反转矩阵明确为 factorial/triangulation 而非独立复制（`1` 个 four-layer-complete cell，
   `all_cells_four_layer_complete=false`，无 3/4 统计规则），`training_only_v3` 的复现层级明确为
   `L1` 聚合可重复。

COJA 的定位不变：有效方法与可学习性正对照，任何文档不得称其失败。ContextWorld 的三份 benchmark
文档本轮未发现与上述收紧直接冲突的“弱信号是统一根因”表述，故不在本次修改范围内。

### 5.65 Motion D2 预注册与 query-action 路由裁决

2026-09-02 新建 [`D2_CONSTRUCTION_PLAN_ZH.md`](D2_CONSTRUCTION_PLAN_ZH.md) 与
[`configs/pusht_motion_damping_d2_preregistration_v1.yaml`](configs/pusht_motion_damping_d2_preregistration_v1.yaml)。
唯一 claim 是 Motion-LeWM 的 **loss-native, data-designed**：不改模型、native objective、初始化与
预算，只检验新 matched training distribution 是否足以诱导自然 held-out history use。D2 不替换
COJA，不升级 benchmark v2，Public Test 继续锁定。

P0 只读 CPU 探针裁掉了无效物理分支：在现有 base template 上，接触自由 query action（已测轴向
幅值最高 `0.5`）的 state condition-gap difference-of-differences 逐分量为 `0`；朝向 block、产生
接触的 query action 才给出非零 block position/velocity `Gamma`。因此 D2-v1 固定为**全零 prefix、
canonical `x2` 后的受控 query contact、contact friction 恒为 `0.25`**，不增加 history excitation，
也不先写数值反解器。该即时探针没有独立 artifact，只承担 P0 路由裁决；P1a 必须在更多
Training-only geometry 上正式复现并生成回执，才可进入构造。

为避免 pilot 自证成功，P1 预分为 `192` calibration + `192` sealed holdout groups：门槛只能由冻结
D0/D1 基线和 calibration 的测量噪声/自然参考范围确定，禁止依据已观察到的 D2 效应下调；冻结后
holdout 只开启一次。`Gamma` 同时报告 RGB pixel energy 与 12 维物理分量，不把 px、px/s、rad、rad/s
硬合成单标量。arm C 的 global sample shuffle 只在完整 model example/clip 层发生，绝不打乱 Lance
时间行；这样首发格不依赖 group co-batching 或 pair metadata。
...
### 7.1 Motion D2-P1a：受控接触物理路由通过，P1b 尚未开始

2026-09-02 在冻结 D2 结构合同后，运行 Training-only CPU 探针
[`probe_pusht_motion_damping_d2_p1a_v1.py`](scripts/probe_pusht_motion_damping_d2_p1a_v1.py)。它不加载
模型、不创建 optimizer、不访问 Development/Public Test；research-local instrumentation 逐行复现
`PushT.step` 的 10 个 physics substeps，并与原生 step 的终态、像素和 raw contact counters 精确一致。

| 方向 | canonical `x2` 最大差 | contact-free `max|Gamma_phys|` | contact `max|Gamma_phys|` | `Gamma_pixel_energy` | 首次物理接触 | same-damping null |
|---|---:|---:|---:|---:|---|---:|
| forward | `1.08e-12` | `0` | `36.4698` | `1.1912e-3` | query raw `2`（0-based）, substep `8` | `0` |
| reverse | `9.38e-13` | `0` | `36.4698` | `1.1731e-3` | query raw `2`（0-based）, substep `8` | `0` |

两种 damping 在两方向都于同一 raw step 与 physics substep 首次接触；history contact 为零、最小
agent-block clearance 为 `65`，query 起点 arbiter 为零，effective contact friction 恒为 `0.25`。
planner-support 四个 scale 中 `0/0.25/0.625` 通过 `L2<=1` 与旋转不裁剪门，`1.0` 被结构门拒绝；
contact-free scale `0.25` 的物理/像素 Gamma 与 same-damping comparator 都是精确零，scale `0.625`
产生非零、镜像一致的局部 approach-axis block response。

正式收据为
[`training_only_cpu_probe_v2.json`](artifacts/pusht_motion_damping_d2_p1a_v1/training_only_cpu_probe_v2.json)，
SHA256 `e2453321c4b76f43ffc65a287d7df08094396be114f927bbb5793d0f812dbd42`；独立目录复跑逐字节一致，
focused CPU tests 为 `4 passed`。**该结果只通过 P1a 物理可行性门**，不能写成 D2-0、数据分布或
